...
const MOMENT_LIMIT = START + 1000 * 60;
console.log(START);
...
function swapSomePair(solve) {
    let numberOfPair = Math.floor(
        Math.sqrt(data.dimension) +
            Math.random() * Math.sqrt(data.dimension) -
            Math.random() * Math.sqrt(data.dimension)
    );
    ...
    const capacities = solve.map(route =>
        route.reduce(
            (capacity, currentVertice) =>
                capacity + parseInt(getVerticeData(currentVertice).demand),
            0
        )
    );
    return capacities;
