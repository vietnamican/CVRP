...
function calculateEuclid(verticeA, verticeB) {
    ...
    return Distance.calculateEuclid(getVerticeData(verticeA), getVerticeData(verticeB));
}
//----------------------------------------------------------------------------------
...
function removeAVertice(solve, cost, routeIndex, verticeIndex, capacities, relocateVertice) {
    const loss = calculateEuclid(solve[routeIndex][verticeIndex - 1], relocateVertice) + calculateEuclid(relocateVertice, solve[routeIndex][verticeIndex + 1]) - calculateEuclid(solve[routeIndex][verticeIndex - 1], solve[routeIndex][verticeIndex + 1]);
    return loss;
}

function addAVertice(solve, cost, routeIndex, verticeIndex, capacities, relocateVertice) {
    const gain = calculateEuclid(solve[routeIndex][verticeIndex - 1], relocateVertice) + calculateEuclid(relocateVertice, solve[routeIndex][verticeIndex]) - calculateEuclid(solve[routeIndex][verticeIndex - 1], solve[routeIndex][verticeIndex]);
    return gain;
}

function commitRemove(solve, cost, routeIndex, verticeIndex, capacities, relocateVertice) {
    solve[routeIndex].splice(verticeIndex, 1);
    capacities -= parseInt(getVerticeData(relocateVertice).demand)
}

function commitAdd(solve, cost, routeIndex, verticeIndex, capacities, relocateVertice) {
...
//----------------------------------------------------------------------------------
function relocate(solve, cost, relocateRouteIndex, relocateVerticeIndex, capacities) {
    let isImprove = false;
    ...
    // remove a vetice
    const loss = removeAVertice(removeSolve, cost, relocateRouteIndex, relocateVerticeIndex, removeCapacities, relocateVertice);
    commitRemove(removeSolve, cost, relocateRouteIndex, relocateVerticeIndex, removeCapacities, relocateVertice);
    ...
    for (let i = 0; i < tempSolve.length; i++) {
        for (let j = 0; j <= tempSolve[i].length; j++) {
            const currentDemand = parseInt(getVerticeData(relocateVertice).demand);
            if (tempCapacities[i] + currentDemand < data.capacity) {
                let gain = addAVertice(tempSolve, cost, i, j, tempCapacities, relocateVertice);
                if (gain < minGain) {
                    minGain = gain;
                    minRoute = i;
                    minVertice = j;
    ...
    if (isImprove && cost - loss + minGain < cost) {
        ...
        commitAdd(removeSolve, cost, minRoute, minVertice, removeCapacities, relocateVertice);
...
function main() {
    ...
    let currentSolve = clone(solve);
    let currentCapacities = clone(capacities);
    console.log(solve);
    console.log(cost);
    ...
    do {
        const [improveSolve, improveSolveCost, improveCapacities] = localSearch(solve, cost, capacities);
        ...
        countSearch++;
    } while (countSearch < MAX_LOCALSEARCH && countSearchNotImprove < MAX_LOCALSEARCH_UNIMPROVE);
    console.log(solve);
